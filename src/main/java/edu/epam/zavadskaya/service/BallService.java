...
public class BallService {
    ...
    private static final BallService INSTANCE = new BallService();
    ...
    public double findArea(Ball ball){
        return 4 * Math.PI * Math.pow(ball.getRadius().getRadius(), 2);
    }

    //объема шара
    public double findVolume(Ball ball){
        return 4 / 3.0 * Math.PI * Math.pow(ball.getRadius().getRadius(), 3);
    }
    ...
    public boolean isTouch(Ball ball){
        double spherePointCoordinateX = ball.getCenter().getX() - ball.getRadius().getRadius();
        double spherePointCoordinateY = ball.getCenter().getY() - ball.getRadius().getRadius();
        double spherePointCoordinateZ = ball.getCenter().getZ() - ball.getRadius().getRadius();
